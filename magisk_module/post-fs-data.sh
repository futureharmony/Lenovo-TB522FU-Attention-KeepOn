...
##########################################################################################
# 4.（已裁撤）Framework-res RRO idmap 预生成 —— v1.8.6 移除。
#    该 overlay 从未生效（MATCH_SYSTEM_ONLY 解析 + 复用 ROM overlay 包名的签名冲突），
#    真实绑定链路 = cmd attention setTestableAttentionService（见 attention_ctrl 头部）。
#    保留 keep_on_looking.xml 写入（ColorOS 设置界面开关状态的持久化，成本低、可回退）。
##########################################################################################
